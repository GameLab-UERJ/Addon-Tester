@tool
extends Node
## Used to trigger bullet spawning from SpawnPoints, bullets, or anything else.
## A trigger is made of a condition + an event : if the condition is met, the event 
## is triggered. Ex : spawn pattern 2 when a bullet is at less than 100 pixels of the player.
