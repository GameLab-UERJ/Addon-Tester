@tool
extends Path2D
## The bullet properties. Determines how the bullet will act once spawned.
## Basically : type / movement / …
class_name BulletPattern

@export var id:String = ""					##1. Nodes are linked with each other using a ID variable.[br]2. This ID can be whatever string you want.[br]3. An ID cannot reference multiple nodes of the same type in the same scene. ex :[br]- 2 [**BulletPattern**](https://www.notion.so/BulletPattern-5acdf89f0d5841a383a67358623066f5?pvs=21)  cannot have the same ID[br]- A [**BulletPattern**](https://www.notion.so/BulletPattern-5acdf89f0d5841a383a67358623066f5?pvs=21) and a [**TriggerContainer**](https://www.notion.so/TriggerContainer-c17ca2e5d106471fbbff530b64235763?pvs=21)  can have the same ID.[br]- 2 [**BulletPattern**](https://www.notion.so/BulletPattern-5acdf89f0d5841a383a67358623066f5?pvs=21) can have the same ID if they are in different scenes that never interact with each others (still, not recommended)[br]4. If a node tries to get another node with its ID but this ID doesn’t reference any node, it will cause an error.[br]5. If you have a scene with some of the nodes/resources having IDs, and that scene needs to be instanced multiple times, it will cause an error because it means they will be multiple nodes of the same ID in the same scene. To avoid that, you can change the ID dynamically when the scene is instanced. But that can only be done in the `_enter_tree()` function.[br][br]If you don’t know how to use `_enter_tree()`, watch
