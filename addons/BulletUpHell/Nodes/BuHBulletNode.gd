extends Area2D
## A node that acts like a default bullet, bound to a BulletPattern.
## In case you need a bullet that requires a very specific thing that needs to be in a scene or 
## you need a bullet with multiple collision shapes.
