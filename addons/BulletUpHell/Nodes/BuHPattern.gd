@tool
extends Path2D
## Determines what we generally call the “pattern” of a group of bullets.
## Basically : the bullets’ spawn position + initial direction.
class_name SpawnPattern

@export var id:String = ""
@export var pattern:Pattern										## Pattern of the spawned bullets
...
func _ready():
	if not Engine.is_editor_hint() and pattern:
		if pattern.forced_target: 
			pattern.node_target = get_node(pattern.forced_target)
